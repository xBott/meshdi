...
public class ModuleMetaProcessor extends AbstractMetaProcessor {
    ...
    protected void configure(ProcessorConfigurationBuilder builder) {
        ...
        builder.select(ModelKind.CLASS, Module.class)
                ...
                .validate(rules -> rules
                        ...
                        .rule(representation -> {
                            try {
                                Semver.parse(representation.version());
                                return ok();
                        ...
                        .rule(representation -> {
                            try {
                                RangeListFactory.create(representation.apiVersion());
                                return ok();
    ...
    private void generateDependencies(
        ...
        for (int i = 0; i < dependencies.size(); i++) {
            ...
            methodSpec.addStatement("$T $L = new $T($S, $T.$L, $T.$L, $S)",
                    requestClassName,
                    dependencyName,
                    requestClassName,
                    dependency.id(),
    ...
    private void generateDescriptor(ModuleRepresentation representation) {
        ...
        typeSpec.addField(
                FieldSpec.builder(Semver.class, "version")
                        .addModifiers(Modifier.PRIVATE, Modifier.FINAL)
                        .initializer("$T.parse($S)", Semver.class, representation.version())
                        .build()
        );

        typeSpec.addField(
                FieldSpec.builder(RangeList.class, "apiVersion")
                        .addModifiers(Modifier.PRIVATE, Modifier.FINAL)
                        .initializer("$T.create($S)", RangeListFactory.class, representation.apiVersion())
                        .build()
        );
        ...
        typeSpec.addMethod(
                MethodSpec.methodBuilder("semver")
                        .addAnnotation(Override.class)
                        .addModifiers(Modifier.PUBLIC)
                        .addStatement("return version")
                        .returns(Semver.class)
                        .build()
        );
        typeSpec.addMethod(
